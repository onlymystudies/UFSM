/* Arquivo abp.c Leonardo 202012828 */
#include <stdio.h>
#include <stdlib.h>
...
int cont = 0;
void conta_descendentes (Arv* a) {
  if (a != NULL) {
    conta_descendentes(a->esq);
    cont++;
    conta_descendentes(a->dir);
  }
}
int numdeNodos = 0;
void imprime2 (Arv* a)
{
  if (a != NULL) {
    numdeNodos++;
    imprime2(a->esq);
    conta_descendentes(a);
    printf("%d possui %d descendentes\n",a->info, cont - 1);
    cont = 0;
    imprime2(a->dir);
  }
}
...
int main(void) {
  ...
  imprime2 (arv); puts("");
  printf("Esta arvore contem %d nodos\n", numdeNodos);  numdeNodos = 0; puts("");
  arv = insere(arv, 1);
  arv = insere(arv, 4);
  arv = insere(arv, 3);
  imprime_alt(arv); puts("");
