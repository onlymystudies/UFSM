/* Problema 2
Implementar o algoritmo da pág. 161: “proc profr (v:nó)_”, versão recursiva:
proc profr (v:nó)_
var w:nó;
inicio
execute visite(v);
...
se !marcado(w) entao execute profr(w)
fim
Problema 3
Implementar, em C, os seguintes procedimentos, das páginas 160 e 161:
proc ampl (v:nó)_
proc prof (V:nó)_ */
#include <stdio.h>
#include <stdlib.h>
...
void buscaAmplitude(Graph G, vertex v) { // proc ampl (v:nó)_
    if (v < G->V) {
        for (size_t i = 0; i < G->V; ++i) {
...
void buscaProfundidade(Graph G, vertex v) { // proc prof (V:nó)_
    if (v < G->V) {
        for (size_t i = 0; i < G->V; ++i) {
            for (link a = G->adj[i]; a != NULL; a = a->next) {
                printf( "%2d", a->w);
...
void buscaProfundidadeRecursiva(Graph G, vertex v) { // proc profr (v:nó)_
    link a;
    if (v < G->V) {
        for (a = G->adj[v]; a != NULL; a = a->next) {
            printf( "%2d", a->w);	// visita
            a->marcado = 1;	// 0 nao e 1 marcado
        }
        puts("");
        v++;
        buscaProfundidadeRecursiva(G, v); // executa profr...
    }
}
...
//5: 3
    ...
    GRAPHshow(G);       puts("");
    GRAPHshowAll(G);    puts("");
    buscaProfundidadeRecursiva(G, 0);   puts("");
    buscaProfundidade(G, 0);            puts("");
    buscaAmplitude(G, 0);               puts("");
    system("pause");
    return 0;
